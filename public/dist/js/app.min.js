...
Plugin.call($spy,data)})})}(jQuery);;(function(){'use strict';function FastClick(layer,options){var oldOnClick;options=options||{};this.trackingClick=false;this.trackingClickStart=0;this.targetElement=null;this.touchStartX=0;this.touchStartY=0;this.lastTouchIdentifier=0;this.touchBoundary=options.touchBoundary||10;this.layer=layer;this.tapDelay=options.tapDelay||200;this.tapTimeout=options.tapTimeout||700;if(FastClick.notNeeded(layer)){return;}
...
return false;};FastClick.attach=function(layer,options){return new FastClick(layer,options);};if(typeof define==='function'&&typeof define.amd==='object'&&define.amd){define(function(){return FastClick;});}else if(typeof module!=='undefined'&&module.exports){module.exports=FastClick.attach;module.exports.FastClick=FastClick;}else{window.FastClick=FastClick;}}());(function($){$.fn.extend({slimScroll:function(options){var defaults={width:'auto',height:'250px',size:'7px',color:'#000',position:'right',distance:'1px',start:'top',opacity:.4,alwaysVisible:false,disableFadeOut:false,railVisible:false,railColor:'#333',railOpacity:.2,railDraggable:true,railClass:'slimScrollRail',barClass:'slimScrollBar',wrapperClass:'slimScrollDiv',allowPageScroll:false,wheelStep:20,touchScrollStep:200,borderRadius:'7px',railBorderRadius:'7px'};var o=$.extend(defaults,options);this.each(function(){var isOverPanel,isOverBar,isDragg,queueHide,touchDif,barHeight,percentScroll,lastScroll,divS='<div></div>',minBarHeight=30,releaseScroll=false;var me=$(this);if(me.parent().hasClass(o.wrapperClass))
...
el.data('daterangepicker').remove();el.data('daterangepicker',new DateRangePicker(el,options,cb));});return this;};}));if(typeof jQuery==="undefined"){throw new Error("AdminLTE requires jQuery");}
...
function done(box){box.find(overlay).remove();settings.onLoadDone.call(box);}};})(jQuery);(function($){'use strict';$.fn.activateBox=function(){$.AdminLTE.boxWidget.activate(this);};})(jQuery);(function($){'use strict';$.fn.todolist=function(options){var settings=$.extend({onCheck:function(ele){return ele;},onUncheck:function(ele){return ele;}},options);return this.each(function(){if(typeof $.fn.iCheck!='undefined'){$('input',this).on('ifChecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onCheck.call(ele);});$('input',this).on('ifUnchecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onUncheck.call(ele);});}else{$('input',this).on('change',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");if($('input',ele).is(":checked")){settings.onCheck.call(ele);}else{settings.onUncheck.call(ele);}});}});};}(jQuery));Vue.filter('momentify',function(value){return moment(value).format("DD.MM.YYYY");});Vue.filter('invoiceslug',function(value){return moment(value).subtract(1,'month').format("YYYY-MM-DD")+'+'+moment(value).format("YYYY-MM-DD");});Vue.component('diffbot-invoice',{template:`<div class="invoice {{ inv.status }}"><h4 class="invoice-title"><a v-link="{ name: 'chart', params: {from: from, to: to} }"
title="Show on chart">{{inv.date.date|momentify}}</a></h4><div class="sexy_line"></div><span class="amount">{{inv.totalAmount|currency}}</span><span class="amount smaller overage"
v-if="inv.overageAmount > 0">of which overage:{{inv.overageAmount|currency}}!</span><span class="amount smaller">{{inv.totalCalls}}calls</span><div class="sexy_line"></div><span class="plan">{{inv.plan|capitalize}}plan</span><div class="price_badge"title="paid"v-if="inv.status == 'paid'"><i class="fa fa-thumbs-up"></i></div><div class="price_badge yella"title="unpaid"
v-if="inv.status != 'paid'"><i class="fa fa-exclamation-circle"></i></div></div>`,props:['inv'],data:function(){return{from:moment(this.inv.date.date).subtract(1,'month').format("YYYY-MM-DD"),to:moment(this.inv.date.date).format("YYYY-MM-DD")}}});Vue.component('tokenform',{template:`<form@submit.prevent.stop action="#"method="get"
class="sidebar-form"><div class="input-group"><input type="{{ type }}"autocomplete="off"name="q"
id="tokenInput"
class="form-control"
value="{{token}}"
placeholder="Enter token"@keyup.enter="tokensave"><span class="input-group-btn"><!--
...
--><input type="submit"value="Whatever"style="display: none;"><button@click="tokenshow"type="submit"name="search"
id="unmask-btn"
class="btn btn-flat"><i class="ion-eye"></i></button><button@click="tokensave"type="submit"name="search"
id="search-btn"
class="btn btn-flat"><i class="ion-checkmark"></i></button></span></div></form>`,props:['token'],data:function(){return{type:"password"}},methods:{tokenshow:function(){this.type=(this.type=="password")?"text":"password";},tokensave:function(event){event.target.blur();this.$dispatch('token-saved',document.getElementById('tokenInput').value);}}});var Dashboard=Vue.extend({template:`<div class="content-wrapper"><!--Content Header(Page header)--><section class="content-header"><h1>Diffbot Account Dashboard<small>API calls per date,invoice information and status</small></h1></section><!--Main content--><section class="content"><template v-if="info.name"><span>Information for{{info.name}},on{{info.plan}}plan</span><h4>API calls frequency over the given period.Change period with range picker below.</h4></template><input v-show="info.name"id="rangepicker"type="text"
placeholder="Data for last 31 days"/><div id="dashchart"></div><div v-if="info.invoices"><h4>Invoice data(click date to render period on chart)</h4><div class="invoice-container"><template v-for="invoice in info.invoices"><diffbot-invoice:inv="invoice"></diffbot-invoice></template></div></div></section><!--/.content--></div>`,props:['info','options'],ready:function(){this.$nextTick(function(){this.initRangePicker();}.bind(this));if(this.info.calls){this.$parent.chartData(this.$parent._filterCallRange(jQuery.extend(true,{},this.info.calls)));}},route:{data:function(){if(this.$route.params.from){this.$dispatch('request-data',this.info.token,this.$route.params);}}},events:{'token-saved':function(token){this.initRangePicker();}},methods:{initRangePicker:function(){$("#rangepicker").daterangepicker({locale:{format:this.options.dateFormat},startDate:this.options.callsChart.startDate,endDate:this.options.callsChart.endDate,ranges:{'Last 7 Days':[moment().subtract(6,'days'),moment()],'Last 31 Days':[moment().subtract(30,'days'),moment()],'Last 6 months':[moment().subtract(6,'months'),moment()],'This year':[moment().startOf('year'),moment()],'Last year':[moment().subtract(1,'year').startOf('year'),moment().subtract(1,'year').endOf('year')],'This Month':[moment().startOf('month'),moment()],'Last Month':[moment().subtract(1,'month').startOf('month'),moment().subtract(1,'month').endOf('month')]},showDropdowns:true,maxDate:moment(),autoApply:true}).on('apply.daterangepicker',function(ev,picker){this.$dispatch('request-data',this.info.token,this.options.callsChart.days,picker);}.bind(this));}}});var Something=Vue.extend({template:`<div class="content-wrapper"><!--Content Header(Page header)--><section class="content-header">Hello there!</section></div>`});var defaultDays=31;var defaultInfo={name:null,plan:null,token:null,range:null,calls:null,invoices:null};Vue.config.debug=true;var App=Vue.extend({data:function(){return{info:defaultInfo,options:{overlay:false,dateFormat:"DD.MM.YYYY.",apiUrl:"/api.php",callsChart:{"days":defaultDays,"startDate":moment().subtract(defaultDays,'days'),"endDate":moment()}}}},ready:function(){this._loadData();if(this.info.token){this._requestData(this.info.token,this.options.callsChart.days);}},events:{'token-saved':function(token){localStorage.clear();this._requestData(token);this.$broadcast('token-saved',token);},'request-data':function(token,days,picker){if(typeof days==='object'&&days!==null){this.options.callsChart.startDate=moment(days.from);this.options.callsChart.endDate=moment(days.to);this.options.callsChart.days=Math.abs(this.options.callsChart.startDate.diff(moment(),'days')-1);}else{this.options.callsChart.days=Math.abs(picker.startDate.diff(moment(),'days')-1);this.options.callsChart.startDate=picker.startDate;this.options.callsChart.endDate=picker.endDate;}
this._requestData(token,this.options.callsChart.days);},'hello':function(msg){console.log(msg);}},methods:{_loadData:function(){var cached=localStorage.getItem('cached');if(cached!==null){cached=JSON.parse(cached);}
this.info=jQuery.extend(true,this.info,cached);},_requestData:function(token,days){if(days===undefined||days===null){days=31;}
this.showOverlay(true);if(this._checkRefreshNeeded(days)){this.$http.get(this.options.apiUrl,{"token":token,"days":days},function(data,status,request){if(data.status=="OK"){data['data']['token']=token;this._saveData(data['data']);var filtered=this._filterCallRange(jQuery.extend(true,{},data['data']['calls']));this.chartData(filtered);}else{localStorage.clear();this.info=defaultInfo;swal("Oh noes!","Looks like something went wrong: "+data.message+" (code: "+data.code+")","error");}
this.showOverlay(false);}.bind(this));}else{var filtered=this._filterCallRange(jQuery.extend(true,{},this.info.calls));this.chartData(filtered);this.showOverlay(false);}},_checkRefreshNeeded:function(days){var refreshNeeded=false;if(this.info.range===null){return true;}
var nowFormatted=moment().format('YYYY-MM-DD');var toFormatted=moment(this.info.range.to).format('YYYY-MM-DD');var fromDiff=Math.abs(moment(this.info.range.from).diff(moment(nowFormatted),'days'));if(moment(nowFormatted).isAfter(moment(toFormatted))||fromDiff<days){refreshNeeded=true;}
return refreshNeeded;},_saveData:function(data){localStorage.setItem('cached',JSON.stringify(data));this._loadData();},_filterCallRange:function(calls){var keys=Object.keys(calls);var dlen=keys.length;var startCheck=moment(this.options.callsChart.startDate.format()).subtract(1,'days');var endCheck=this.options.callsChart.endDate;for(var i=0;i<dlen;i++){if(!moment(keys[i]).isBetween(startCheck,endCheck)){delete calls[keys[i]];}}
return calls;},chartData:function(data){var vals=Object.keys(data).map(function(key){return data[key];});c3.generate({bindto:'#dashchart',data:{x:'x',type:"bar",columns:[['x'].concat(Object.keys(data)),['Number of calls'].concat(vals)],axes:{data1:'y'}},axis:{x:{type:'timeseries',tick:{format:'%d.%m.%Y.'}}}});},showOverlay:function(bool){this.options.overlay=bool;}},http:{root:'/'}});var router=new VueRouter({hashbang:false,linkActiveClass:'active'});router.map({'/dashboard':{component:Dashboard},'/dashboard/chart/:from/:to':{name:'chart',component:Dashboard},'/extra':{component:Something}});router.redirect({'/':'/dashboard'});router.start(App,'#app');