...
function done(box){box.find(overlay).remove();settings.onLoadDone.call(box);}};})(jQuery);(function($){'use strict';$.fn.activateBox=function(){$.AdminLTE.boxWidget.activate(this);};})(jQuery);(function($){'use strict';$.fn.todolist=function(options){var settings=$.extend({onCheck:function(ele){return ele;},onUncheck:function(ele){return ele;}},options);return this.each(function(){if(typeof $.fn.iCheck!='undefined'){$('input',this).on('ifChecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onCheck.call(ele);});$('input',this).on('ifUnchecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onUncheck.call(ele);});}else{$('input',this).on('change',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");if($('input',ele).is(":checked")){settings.onCheck.call(ele);}else{settings.onUncheck.call(ele);}});}});};}(jQuery));var ti=document.getElementById('tokenInput');var format="DD.MM.YYYY.";var info={"name":null,"plan":null,"token":null,"days":31,"callsChart":{"startDate":moment().subtract(31,'days'),"endDate":moment()}};Vue.config.debug=true;var vm=new Vue({el:'#app',data:{info:info,ui:{overlay:false}},ready:function(){this.info=jQuery.extend(true,this.info,this.loadData());if(this.info.token){this.requestData(this.info.token,this.info.days);}
this.$nextTick(function(){vm.initRangePicker();});},methods:{tokensave:function(){this.saveData();this.requestData(ti.value);this.initRangePicker();},tokenshow:function(){ti.type=(ti.type=="password")?"text":"password";},initRangePicker:function(){$("#rangepicker").daterangepicker({locale:{format:format},startDate:vm.info.callsChart.startDate,endDate:vm.info.callsChart.endDate,ranges:{'Last 7 Days':[moment().subtract(6,'days'),moment()],'Last 31 Days':[moment().subtract(30,'days'),moment()],'Last 6 months':[moment().subtract(6,'months'),moment()],'This year':[moment().startOf('year'),moment()],'Last year':[moment().subtract(1,'year').startOf('year'),moment().subtract(1,'year').endOf('year')],'This Month':[moment().startOf('month'),moment()],'Last Month':[moment().subtract(1,'month').startOf('month'),moment().subtract(1,'month').endOf('month')]},showDropdowns:true,maxDate:moment(),autoApply:true}).on('apply.daterangepicker',function(ev,picker){vm.info.days=Math.abs(picker.startDate.diff(moment(),'days')-1);vm.info.callsChart.startDate=picker.startDate;vm.info.callsChart.endDate=picker.endDate;vm.requestData(vm.info.token,vm.info.days);});},requestData:function(token,days){if(days===undefined||days===null){days=31;}
this.showOverlay(true);this.$http.get('/api.php',{"token":token,"days":days},function(data,status,request){if(data.status=="OK"){var keys=Object.keys(data['data']['calls']);var dlen=keys.length;for(var i=0;i<dlen;i++){var startCheck=moment(this.info.callsChart.startDate.format()).subtract(1,'days');var endCheck=this.info.callsChart.endDate;if(!moment(keys[i]).isBetween(startCheck,endCheck)){delete data['data']['calls'][keys[i]];}}
data['data']['token']=token;this.saveData(data['data']);this.chartData(data['data']['calls']);}else{this.resetInfo();swal("Oh noes!","Looks like something went wrong: "+data.message+" (code: "+data.code+")","error");}
this.showOverlay(false);});},saveData:function(data){if(data!==undefined){this.saveData();for(var property in data){if(data.hasOwnProperty(property)){localStorage.setItem(property,data[property]);this.info[property]=data[property];}}}else{localStorage.clear();}},loadData:function(){return{"name":localStorage.getItem("name"),"plan":localStorage.getItem("plan"),"token":localStorage.getItem("token"),"days":localStorage.getItem("days")||31}},resetInfo:function(){this.info=info;},showOverlay:function(bool){this.ui.overlay=bool;},chartData:function(data){var vals=Object.keys(data).map(function(key){return data[key];});c3.generate({bindto:'#dashchart',data:{x:'x',type:"bar",columns:[['x'].concat(Object.keys(data)),['Number of calls'].concat(vals)],axes:{data1:'y'}},axis:{x:{type:'timeseries',tick:{format:'%d.%m.%Y.'}}}});}},http:{root:'/'}});