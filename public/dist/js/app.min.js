...
v-if="inv.status != 'paid'"><i class="fa fa-exclamation-circle"></i></div></div>`,props:['inv']});Vue.component('tokenform',{template:`<form@submit.prevent.stop action="#"method="get"
...
this.showOverlay(false);}.bind(this));}else{var filtered=this._filterCallRange(jQuery.extend(true,{},this.info.calls));this.chartData(filtered);this.showOverlay(false);}},_checkRefreshNeeded:function(days){var refreshNeeded=false;var nowFormatted=moment().format('YYYY-MM-DD');var toFormatted=moment(this.info.range.to).format('YYYY-MM-DD');var fromDiff=Math.abs(moment(this.info.range.from).diff(moment(nowFormatted),'days'));if(moment(nowFormatted).isAfter(moment(toFormatted))||fromDiff<days||this.info.range===null){refreshNeeded=true;}
