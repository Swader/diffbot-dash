...
function done(box){box.find(overlay).remove();settings.onLoadDone.call(box);}};})(jQuery);(function($){'use strict';$.fn.activateBox=function(){$.AdminLTE.boxWidget.activate(this);};})(jQuery);(function($){'use strict';$.fn.todolist=function(options){var settings=$.extend({onCheck:function(ele){return ele;},onUncheck:function(ele){return ele;}},options);return this.each(function(){if(typeof $.fn.iCheck!='undefined'){$('input',this).on('ifChecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onCheck.call(ele);});$('input',this).on('ifUnchecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onUncheck.call(ele);});}else{$('input',this).on('change',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");if($('input',ele).is(":checked")){settings.onCheck.call(ele);}else{settings.onUncheck.call(ele);}});}});};}(jQuery));Vue.filter('momentify',function(value){return moment(value).format("DD.MM.YYYY");});var Something=Vue.extend({template:`<div class="content-wrapper"><!--Content Header(Page header)--><section class="content-header">Hello there!</section></div>`});Vue.filter('invoiceslug',function(value){return moment(value).subtract(1,'month').format("YYYY-MM-DD")+'+'+moment(value).format("YYYY-MM-DD");});Vue.component('diffbot-invoice',{template:`<div class="invoice {{ inv.status }}"><h4 class="invoice-title"><a v-link="{ name: 'chart', params: {from: from, to: to} }"
...
v-if="inv.status != 'paid'"><i class="fa fa-exclamation-circle"></i></div></div>`,props:['inv'],data:function(){return{from:moment(this.inv.date.date).subtract(1,'month').format("YYYY-MM-DD"),to:moment(this.inv.date.date).format("YYYY-MM-DD")}}});var Dashboard=Vue.extend({template:`<div class="content-wrapper"><!--Content Header(Page header)--><section class="content-header"><h1>Diffbot Account Dashboard<small>API calls per date,invoice information and status</small></h1></section><!--Main content--><section class="content"><template v-if="!sharedState.token">Use the input field on the left to enter a token to analyze.</template><template v-if="sharedState.cached"><span>Information for{{sharedState.cached.name}},on{{sharedState.cached.plan}}plan</span><h4>API calls frequency over the given period.Change period with range picker below.</h4></template><input v-show="sharedState.cached"id="rangepicker"type="text"
placeholder="Data for last 31 days"/><div id="dashchart"></div><div v-if="sharedState.cached"><h4>Invoice data(click date to render period on chart)</h4><div class="invoice-container"><template v-for="invoice in sharedState.cached.invoices"><diffbot-invoice:inv="invoice"></diffbot-invoice></template></div></div></section><!--/.content--></div>`,ready:function(){this.$nextTick(function(){this.initRangePicker();});},data:function(){return{sharedState:store.state,privateState:{}}},route:{data:function(){if(this.sharedState.token){if(this.$route.params.from){this.$dispatch('request-data',this.$route.params);}else{this.$dispatch('request-data',{from:moment().subtract(31,'days').format('YYYY-MM-DD'),to:moment().format('YYYY-MM-DD')});}}else{router.go('/dashboard');}}},events:{'data-refreshed':function(data){var filteredCalls=this._filterCallRange(jQuery.extend(true,{},data['calls']));this.$nextTick(function(){this.chartData(filteredCalls);});}},methods:{initRangePicker:function(){$("#rangepicker").daterangepicker({locale:{format:store.config.dateFormat},startDate:store.config.callsChart.startDate,endDate:store.config.callsChart.endDate,ranges:{'Last 7 Days':[moment().subtract(6,'days'),moment()],'Last 31 Days':[moment().subtract(30,'days'),moment()],'Last 6 months':[moment().subtract(6,'months'),moment()],'This year':[moment().startOf('year'),moment()],'Last year':[moment().subtract(1,'year').startOf('year'),moment().subtract(1,'year').endOf('year')],'This Month':[moment().startOf('month'),moment()],'Last Month':[moment().subtract(1,'month').startOf('month'),moment().subtract(1,'month').endOf('month')]},showDropdowns:true,maxDate:moment(),autoApply:true}).on('apply.daterangepicker',function(ev,picker){router.go({name:'chart',params:{from:picker.startDate.format("YYYY-MM-DD"),to:picker.endDate.format("YYYY-MM-DD")}});}.bind(this));},_filterCallRange:function(calls){var keys=Object.keys(calls);var dlen=keys.length;var startCheck=moment(store.config.callsChart.startDate).subtract(1,'days');var endCheck=store.config.callsChart.endDate;for(var i=0;i<dlen;i++){if(!moment(keys[i]).isBetween(startCheck,endCheck)){delete calls[keys[i]];}}
return calls;},chartData:function(data){var vals=Object.keys(data).map(function(key){return data[key];});c3.generate({bindto:'#dashchart',data:{x:'x',type:"bar",columns:[['x'].concat(Object.keys(data)),['Number of calls'].concat(vals)],axes:{data1:'y'}},axis:{x:{type:'timeseries',tick:{format:'%d.%m.%Y.'}}}});},}});Vue.component('tokenform',{template:`<form@submit.prevent.stop action="#"method="get"
...
id="search-btn"
class="btn btn-flat"><i class="ion-checkmark"></i></button></span></div></form>`,props:['token'],data:function(){return{type:"password"}},methods:{tokenshow:function(){this.type=(this.type=="password")?"text":"password";},tokensave:function(event){event.target.blur();this.$dispatch('token-saved',document.getElementById('tokenInput').value);}}});Vue.config.debug=true;var store={config:{dateFormat:"DD.MM.YYYY.",apiUrl:"/api.php",callsChart:{"days":31,"startDate":moment().subtract(31,'days'),"endDate":moment()}},state:{overlay:false,token:localStorage.getItem('token'),cached:JSON.parse(localStorage.getItem('cached'))},showOverlay:function(bool){this.state.overlay=(bool===true);},updateToken:function(token){this.state.token=token;localStorage.clear();localStorage.setItem('token',token);this.cache(null);},cache:function(data){this.state.cached=data;localStorage.setItem('cached',JSON.stringify(data));}};var App=Vue.extend({data:function(){return{sharedState:store.state,privateState:{}}},events:{'token-saved':function(value){store.updateToken(value);this._requestData();},'request-data':function(range){var days=Math.abs(moment(range.from).diff(moment(),'days'))+1;store.config.callsChart.startDate=range.from;store.config.callsChart.endDate=range.to;this._requestData(days);}},ready:function(){if(store.state.token){this._requestData();}},methods:{_requestData(days){if(days===undefined||days===null){days=31;}
store.showOverlay(true);if(this._dataRefreshNeeded(days)){this.$http.get(store.config.apiUrl,{"token":this.sharedState.token,"days":days},function(data,status,request){if(data.status=="OK"){store.cache(data['data']);this.$broadcast('data-refreshed',data['data']);}else{localStorage.clear();swal("Oh noes!","Looks like something went wrong: "+data.message+" (code: "+data.code+")","error");}
store.showOverlay(false);}.bind(this));}else{this.$broadcast('data-refreshed',this.sharedState.cached);store.showOverlay(false);}},_dataRefreshNeeded(days){if(this.sharedState.cached===null||this.sharedState.cached.range===null){return true;}
var nowFormatted=moment().format('YYYY-MM-DD');var toFormatted=moment(this.sharedState.cached.range.to).format('YYYY-MM-DD');var fromDiff=Math.abs(moment(this.sharedState.cached.range.from).diff(moment(nowFormatted),'days'));return!!(moment(nowFormatted).isAfter(moment(toFormatted))||fromDiff<(days-1));}}});var router=new VueRouter({hashbang:false,linkActiveClass:'active'});router.map({'/dashboard':{component:Dashboard},'/dashboard/chart/:from/:to':{name:'chart',component:Dashboard},'/extra':{component:Something}});router.redirect({'/':'/dashboard'});router.start(App,'#app');