...
if(typeof define==='function'&&define.amd){define(function(){return sweetAlert;});}else if(typeof module!=='undefined'&&module.exports){module.exports=sweetAlert;}})(window,document);if(typeof jQuery==="undefined"){throw new Error("AdminLTE requires jQuery");}
...
function done(box){box.find(overlay).remove();settings.onLoadDone.call(box);}};})(jQuery);(function($){'use strict';$.fn.activateBox=function(){$.AdminLTE.boxWidget.activate(this);};})(jQuery);(function($){'use strict';$.fn.todolist=function(options){var settings=$.extend({onCheck:function(ele){return ele;},onUncheck:function(ele){return ele;}},options);return this.each(function(){if(typeof $.fn.iCheck!='undefined'){$('input',this).on('ifChecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onCheck.call(ele);});$('input',this).on('ifUnchecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onUncheck.call(ele);});}else{$('input',this).on('change',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");if($('input',ele).is(":checked")){settings.onCheck.call(ele);}else{settings.onUncheck.call(ele);}});}});};}(jQuery));var ti=document.getElementById('tokenInput');var info={"name":null,"plan":null,"token":null};var vm=new Vue({el:'#app',data:{info:info,ui:{overlay:false}},ready:function(){this.info=this.loadData();if(this.info.token){this.requestData(this.info.token);}},methods:{tokensave:function(){this.saveData();this.requestData(ti.value);},tokenshow:function(){ti.type=(ti.type=="password")?"text":"password";},requestData:function(token){this.showOverlay(true);this.$http.get('/api.php',{"token":token},function(data,status,request){if(data.status=="OK"){data['data']['token']=token;this.saveData(data['data']);}else{this.resetInfo();swal("Oh noes!","Looks like something went wrong: "+data.message+" (code: "+data.code+")","error");}
this.showOverlay(false);});},saveData:function(data){if(data!==undefined){this.saveData();for(var property in data){if(data.hasOwnProperty(property)){localStorage.setItem(property,data[property]);this.info[property]=data[property];}}}else{localStorage.clear();}},loadData:function(){return{"name":localStorage.getItem("name"),"plan":localStorage.getItem("plan"),"token":localStorage.getItem("token")}},resetInfo:function(){this.info=info;},showOverlay:function(bool){this.ui.overlay=bool;}},http:{root:'/'}});