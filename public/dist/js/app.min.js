...
{bar.fadeOut('slow');rail.fadeOut('slow');}},1000);}}});return this;}});$.fn.extend({slimscroll:$.fn.slimScroll});})(jQuery);if(typeof jQuery==="undefined"){throw new Error("AdminLTE requires jQuery");}
...
function done(box){box.find(overlay).remove();settings.onLoadDone.call(box);}};})(jQuery);(function($){'use strict';$.fn.activateBox=function(){$.AdminLTE.boxWidget.activate(this);};})(jQuery);(function($){'use strict';$.fn.todolist=function(options){var settings=$.extend({onCheck:function(ele){return ele;},onUncheck:function(ele){return ele;}},options);return this.each(function(){if(typeof $.fn.iCheck!='undefined'){$('input',this).on('ifChecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onCheck.call(ele);});$('input',this).on('ifUnchecked',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");settings.onUncheck.call(ele);});}else{$('input',this).on('change',function(){var ele=$(this).parents("li").first();ele.toggleClass("done");if($('input',ele).is(":checked")){settings.onCheck.call(ele);}else{settings.onUncheck.call(ele);}});}});};}(jQuery));var ti=document.getElementById('tokenInput');var vm=new Vue({el:'#app',data:{token:loadData('token')},methods:{tokensave:function(){this.token=ti.value;saveData({token:this.token});},tokenshow:function(){if(ti.type=="password"){ti.type="text";}else{ti.type="password";}},doThis:function(){this.$http.get('/api.php',{},function(data,status,request){console.log(data,status,request);});}},http:{root:'/'}});function saveData(data){if(data!==undefined){saveData();for(property in Object.keys(data))
for(var property in data){if(data.hasOwnProperty(property)){localStorage.setItem(property,data[property]);}}}else{localStorage.clear();}}
function loadData(key){if(key!==undefined){return localStorage.getItem(key)||'';}
