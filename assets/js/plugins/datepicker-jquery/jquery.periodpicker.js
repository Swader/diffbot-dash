/**
 * @preserve jQuery PeriodPicker plugin v1.9.6
...
(function ($, window, document) {
	...
	function PeriodPicker(startinput, options) {
		...
		that.picker = $('<div unselectable="on" class="period_picker_box xdsoft_noselect ' + (options.fullsize ? 'period_picker_maximize' : '') + '" style="">' +
			...
			'<div class="period_picker_head">' +
				(options.title ? '<span class="period_picker_head_title">' + this.i18n('Select period') + '</span>' : '') +
			...
			'<span class="period_button_text">' +
				'<span class="period_button_content_wrapper">' +
					'<span class="period_button_content">' +
						...
						'<span>' + this.i18n('Choose period') + '</span>' +
					'</span>' +
				'</span>' +
			'</span>' +
			'</button>');
		...
		that.month = that.period[0].length ? that.period[0][0].getMonth() + 1 : options.startMonth;
		that.year = that.period[0].length ? that.period[0][0].getFullYear() : options.startYear;
		...
		uniqueid += 1;
		that.init();
		that.addRangeTime(that.period[0][0], that.period[0][1]);
	}
	PeriodPicker.prototype.returnPeriod = function () {
		this.picker.find('input.period_picker_from').val(this.period[0] !== undefined ? this.period[0] : '');
	...
	PeriodPicker.prototype.moveTimeToDate = function () {
		if (this.options.timepicker && this.periodtime.length && this.periodtime[0].length) {
			if (this.period[0][0] !== null && this.period[0][0].format  && this.periodtime[0][0].format) {
				this.period[0][0].setSeconds(this.periodtime[0][0].getSeconds());
				this.period[0][0].setMinutes(this.periodtime[0][0].getMinutes());
				this.period[0][0].setHours(this.periodtime[0][0].getHours());
			}
			if (this.periodtime[0][1] !== null && this.period[0][1] !== null && this.period[0][1].format && this.periodtime[0][1].format) {
				this.period[0][1].setSeconds(this.periodtime[0][1].getSeconds());
				this.period[0][1].setMinutes(this.periodtime[0][1].getMinutes());
				this.period[0][1].setHours(this.periodtime[0][1].getHours());
			}
		}
	};
	PeriodPicker.prototype.getInputsValue = function () {
		var result = [], format;
		if (this.startinput.length && this.period && this.period[0] && this.period[0].length) {
			this.moveTimeToDate();
			format = this.options.timepicker ? this.options.formatDateTime : this.options.formatDate;
			if (this.period[0][0] && this.period[0][0].format && this.startinput.val() !== this.period[0][0].format(format)) {
				result.push(this.period[0][0].format(format));
			}
			if (this.endinput.length && this.period[0][1] && this.period[0][1].format && this.endinput.val() !== this.period[0][1].format(format)) {
				result.push(this.period[0][1].format(format));
			}
		}
		return result;
	};
	...
	PeriodPicker.prototype.getLabel = function () {
		...
		if (this.period[0].length) {
			...
			if (this.period[0][1] === undefined || !this.period[0][1] || this.period[0][1].format === undefined || !this.period[0][1].format || this.period[0][0].format(formats[3]) === this.period[0][1].format(formats[3])) {
				result.push(this.period[0][0].format(formats[0]));
			} else {
				result.push(this.period[0][0].format(this.period[0][0].format('YYYY') !== this.period[0][1].format('YYYY') ? formats[0] : (this.period[0][0].format('M') !== this.period[0][1].format('M') ? formats[2] : formats[1])));
				result.push(this.period[0][1].format(formats[0]));
			}
		}
		return result;
	};
	PeriodPicker.prototype.setLabel = function () {
		...
		if (result.length) {
			...
			this.button.find('.period_button_content').html('<span class="icon_calendar"></span><span>' + '<span>' + this.i18n('Choose period') + '</span>');
		}
	};
		...
		that.timer1 = setTimeout(function () {
			...
			if (that.period.length) {
				...
				that.picker.find('.period_picker_cell').each(function () {
					...
					if (date.inRange(current, that.period[0])) {
						$(this).addClass('period_picker_selected');
					}
				});
				that.picker.find('.period_picker_years_period').css({
					width: Math.floor((that.options.yearSizeInPixels / 365) * moment(that.period[0][1]).diff(that.period[0][0], 'day')) + 'px',
					left: Math.floor((that.options.yearSizeInPixels / 365) * -moment([that.options.yearsPeriod[0], 0, 1]).diff(that.period[0][0], 'day'))
				});
				that.picker.find('input.period_picker_from:not(:focus)').val((that.period[0][0] !== undefined && that.period[0][0]) ? that.period[0][0].format(that.options.formatDate) : '');
				that.picker.find('input.period_picker_to:not(:focus)').val((that.period[0][1] !== undefined && that.period[0][1]) ? that.period[0][1].format(that.options.formatDate) : that.picker.find('input.period_picker_from').val());
	...
	PeriodPicker.prototype.addRangeTime = function (value1, value2) {
		...
		/*if (this.options.timepicker && this.periodtime[0].length) {
			if (this.periodtime[0][0]) {
				this.picker.find('input.period_picker_from_time:not(:focus)').val(this.periodtime[0][0].format(this.options.timepickerOptions.inputFormat));
			}
			if (this.periodtime[0][0]) {
				this.picker.find('input.period_picker_to_time:not(:focus)').val(this.periodtime[0][0].format(this.options.timepickerOptions.inputFormat));
			}
		}*/
	};
	PeriodPicker.prototype.addRange = function (value) {
		...
		if ($.isArray(value)) {
			this.period[0] = [date.parseStr(value[0], this.options.formatDate), date.parseStr(value[1], this.options.formatDate)];
			if (this.period[0][0] === null) {
				this.period[0] = [];
			}
			this.director = 0;
		} else {
			...
			this.period[0][this.options.norange ? 0 : this.director] = date.parseStr(value, this.options.formatDate);
			if (this.period[0][this.director] === null) {
				this.period[0] = [];
				this.highlightPeriod();
				return;
			}
			if (!this.director) {
				this.period[0][1] = this.period[0][this.director];
			}
			if (this.period[0][0] > this.period[0][1]) {
				buff = this.period[0][0];
				this.period[0][0] = this.period[0][1];
				this.period[0][1] = buff;
			}
			this.director = this.director ? 0 : 1;
		}
		if (this.options.norange && this.period[0][0] && this.period[0][1] && this.period[0][1] !== this.period[0][0]) {
			this.period[0][1] = this.period[0][0].clone();
		}
		this.highlightPeriod();
		if (this.options.hideAfterSelect && this.period[0][0] && this.period[0][1] && this.period[0][0] !== this.period[0][1]) {
			this.hide();
		}
	};
	...
	PeriodPicker.prototype.regenerate = function () {
		...
		moment.locale(that.options.lang);
		this.options.cells = [Math.floor((height - 90) / that.options.monthHeightInPixels) || 1, Math.floor(width / that.options.monthWidthInPixels) || 1];
	...
	PeriodPicker.prototype.init = function () {
		var that = this, offset, start, diff, drag, perioddrag, perioddragger, left, headdrag, period_picker_years_selector, period_picker_years_selector_container;
		...
		if (that.options.timepicker) {
			that.picker.find('.period_picker_submit_dates .period_picker_time input')
		...
		if (that.options.navigate) {
			that.picker.find('.xdsoft_navigate').on('click', function (e) {
		...
		that.picker.on('mouseup', function (e) {
			drag = perioddrag = headdrag = period_picker_years_selector_container = false;
		...
		that.picker.css({
			...
			height: this.options.cells[0] * that.options.monthHeightInPixels + 130
		});
	...
	PeriodPicker.prototype.generateCalendars = function (month, year) {
		...
		if (date[1] > that.options.yearsPeriod[1]) {
			year = that.year = that.options.yearsPeriod[1];
			month = that.month = 12 - that.monthcount;
		}
		if (date[1] < that.options.yearsPeriod[0]) {
			year = that.year = that.options.yearsPeriod[0];
			month = that.month = 1;
		}
		...
		function generateWeek() {
			...
			for (k = 0; k < weekdays.length; k += 1) {
				out.push('<th class="' + (that.options.weekEnds.indexOf(k + that.options.dayOfWeekStart > 7 ? (k + that.options.dayOfWeekStart) % 7 : k + that.options.dayOfWeekStart) !== -1 ? 'period_picker_holiday' : '') + '">' + weekdays[k] + '</th>');
			}
			return out.join('');
		}
	...
	PeriodPicker.prototype.generateYearsLine = function () {
		...
		if (this.options.yearsPeriod && $.isArray(this.options.yearsPeriod)) {
			for (y = this.options.yearsPeriod[0]; y <= this.options.yearsPeriod[1]; y += 1, i += 1) {
	...
	$.fn.periodpicker.defaultOptions = {
		...
		monthWidthInPixels: 187,
		monthHeightInPixels: 164,
